...
public class CmdOwnerAdd extends AutomatableCommand {
   ...
   public boolean onAutoEnable(FPlayer player) {
      ...
      FPlayer target = this.argAsBestFPlayerMatch(0, fme);
      player.msg("<i>Automatically trying to add %s<i> to ownerlist.", target.describeTo(fme));
      return true;
   }
   ...
   public boolean onAutoDisable(FPlayer player) {
      FPlayer target = this.argAsBestFPlayerMatch(0, fme);
      player.msg("<i>No longer automatically trying to add %s<i> to ownerlist.", target.describeTo(fme));
      return true;
   }
